...
// A little caching optimisation for when multiple function are called fom the same Netlify Function
async function getSheetRows(sheetId, filter) {
  ...
  if (typeof filter == 'function') {
    ...
    rows = cache.sheetRows[sheetId].filter(filter)
  } else if (typeof filter == 'object') {
    rows = await sheet.getRows(filter) // not cached
  }
  return rows
}
...
exports.updateApplication = async (rowN, columns) => {
  try {
    const row = await getSheetRows(
      process.env.GOOGLE_SPREADSHEET_APPLICATIONS_SHEET_ID, // TODO caching probably fails
      {
        offset: rowN - 2,
        limit: 1,
      },
...
exports.getAssignmentForEmail = async function (email) {
  try {
    const assignmentRow = (
      await getSheetRows(
        process.env.GOOGLE_SPREADSHEET_ASSIGNMENTS_SHEET_ID,
        (row = row.email == email),
      )
    )[0]
    if (!assignmentRow) {
      throw new Error(`No assignment for email "${email}"`)
    }
...
exports.getAssignmentForRoleCountry = async function (role, country) {
  try {
    ...
    if (!assignmentRow) {
      throw new Error(
        `No assignment for role "${role} and country "${country}"`,
      )
    }
    const assignment = assignmentRow
      ? assignmentObjectFromRow(sheet.headerValues, assignmentRow)
...
exports.getEmailForEvent = async function (event) {
  try {
    const emailRow = (
      await getSheetRows(
        process.env.GOOGLE_SPREADSHEET_EMAILS_SHEET_ID,
        (row) => row.event == event,
      )
    )[0]
    if (!emailRow) {
      throw new Error(`No email for event "${event}"`)
    }
